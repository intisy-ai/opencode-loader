...
## Corpus Check
- 63 files · ~50,568 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 539 nodes · 1210 edges · 34 communities (32 shown, 2 thin omitted)
...
- [[_COMMUNITY_loader-runtime.ts|loader-runtime.ts]]
...
- `checkForUpdates()` --calls--> `updateCheckIntervalHours()`  [EXTRACTED]
  core-loader/src/tui.ts → core-loader/src/config.ts
...
Cohesion: 0.09
Nodes (69): coerceGlobal(), loadConfig(), loadGlobalSettings(), loadPlugins(), saveConfig(), savePlugins(), setGlobalSetting(), HOME (+61 more)

### Community 2 - "config.ts"
Cohesion: 0.11
Nodes (33): autoUpdateCheck(), catalogCacheHours(), defaultTab(), loaderName(), loadLoaderConfig(), loadMcpConfig(), migrateConfigs(), num() (+25 more)

### Community 3 - "plugins.ts"
Cohesion: 0.27
Nodes (26): pad(), rule(), stringWidth(), timeAgo(), trunc(), getMarketplaceActions(), selectInstallMethod(), getMcpActions() (+18 more)
...
### Community 6 - "cli.ts"
Cohesion: 0.19
Nodes (22): ACCOUNTS_JSON, accountsByProvider(), deployedHandlers(), doctor(), hasNpx(), LOADER_CONFIG, loadPluginEntries(), main() (+14 more)
...
### Community 20 - "loader-runtime.ts"
Cohesion: 0.39
Nodes (7): ensureBinDir(), ensureOnPath(), ensureOnPathPosix(), ensureOnPathWindows(), getBinDir(), loadUpdater(), runEarlyLaunchHooks()
...
- **Why does `deployCommands()` connect `index.ts` to `tui-extension.ts`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `render()` connect `plugins.ts` to `input.ts`, `account-menu.ts`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `configDir()` connect `tui-extension.ts` to `index.ts`?**
...
- **Should `input.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.09290709290709291 - nodes in this community are weakly interconnected._